...
const SingUp = ({ isOpen, setOpen }) => {
    ...
    const idCheck = (e) => {
        e.preventDefault();
        if (member.id) {
            axios.post("https://port-0-testserver-luj2cle3qhxst.sel3.cloudtype.app/idCheck", { id: member.id }).then((res) => {
    ...
    // 가입하기
     const handleSubmit = (e) => {
        ...
        if (member.id && member.pass && member.name && member.email && member.tel && idchecked) {
            axios.post("https://port-0-testserver-luj2cle3qhxst.sel3.cloudtype.app/singUp", { id: member.id, pass: member.pass, name: member.name, tel: member.tel, email: member.email }).
                then((res) => {
                    console.log('회원가입성공')
